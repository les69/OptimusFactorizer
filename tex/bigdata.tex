...
the system computes each ui independently of the other item factors and computes each vj independently of the other user factors.
It means that the algorithm can update all the ui and vj in parallel. This gives rise to potentially massive parallelization of the algorithm and improve the efficiency.
\subsubsection{Weighted Alternating Least Square}
Weighted alternating least squares(W-ALS) is an improved algorithm of ALS in last section. W-ALS aims to train the recommendation system with implicated dataset. In some situation, users don't rate items exactly with values but through their behaviour, which is called implicated ratings. An example of implicit ratings is that when people are watching TV program, they don't actually rating, but the recommendation system can assign user's ratings basing their watching time on the program. W-ALS introduces different confidence levels for which items are preferred by the user
...
\end{equation}
Here $c_{i,j}$ measures the confidence of the predicted preference $p_{i,j}$. There are many ways to compute the confidence level, but it will out of the discussion in this paper.
...
\begin{enumerate}
    ...
    \item Last but not least we have conducted a test to investigate the different running times between the three different versions of ALS : the LocalALS without using Spark API and the SparkALS partial using Spark API which is partially paralleled, ParalleledALS fully using Spark API which is fully paralleled. Running on a Matrix with 3.900 movies, 6.040 users, both movie and user are supposed to have 10 features while the training has 5 iteration. For the SparkALS, it is run in 2 slices as when I try to run with 5 slice I got some out of Memory error. The running times are shown in table, it is interesting that the partial paralleled version has the worst performance, because it is just partial paralleled without optimization. On the other hand, ParalleledALS sharply improves the running performance with 5 slice, which has the best performance. 
\begin{table}
\centering
